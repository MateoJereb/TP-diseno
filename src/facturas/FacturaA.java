...
public class FacturaA extends JPanel {
	...
	public FacturaA(FacturaDTO factura, Boolean estadiaFacturada) {
	...
	private void tipoFactura() {
		...
		EtiquetaJ letra = new EtiquetaJ("A");
	...
	private void total(FacturaDTO factura) {
		...
		subizq.setBackground(Color.WHITE);
		subizq.setBorder(blackLineBorder);
		
		EtiquetaJ textoSubIzq = new EtiquetaJ("<html><body><p style=\"text-align:right;\">"
				+ "Sub-total<br>"
				+ "I.V.A %</p></body></html>");
		textoSubIzq.setFont(new Font("Calibri",Font.PLAIN,11));
		
		JPanel subder = new JPanel(new GridBagLayout());
		subder.setBackground(Color.WHITE);
		...
		subizq.add(textoSubIzq,cons);
		
		subder.add(textoSubDer,cons);
